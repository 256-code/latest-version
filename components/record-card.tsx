...
export function RecordCard({ record, onOpenTask }: { record: ChangeRecord; onOpenTask: (code: string) => void }) {
  ...
  const confirmVoid = () => {
    if (!reason.trim()) { setError('作废属于高风险操作，必须填写原因。'); return }
    actions.setRecordStatus(record.code, '已作废', reason.trim())
    setVoiding(false)
    setReason('')
    setError('')
  }
          ...
          {canVoid && record.status === '已发布' ? (
            voiding ? (
              <span className="inline-confirm">
                <input value={reason} autoFocus placeholder="填写作废原因（必填）" onChange={event => { setReason(event.target.value); setError('') }} />
                <button type="button" className="secondary-button" onClick={() => { setVoiding(false); setError('') }}>取消</button>
                <button type="button" className="danger-button" onClick={confirmVoid}>确认作废</button>
              </span>
            ) : (
