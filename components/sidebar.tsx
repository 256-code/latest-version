...
function NavButton({ entry, active, count, onClick }: { entry: NavEntry; active: boolean; count?: number; onClick: () => void }) {
  ...
  return (
    <button type="button" className={`nav-item ${active ? 'active' : ''}`} onClick={onClick} title={entry.hint}>
      <Icon size={17} />
...
export function Sidebar() {
  ...
  return (
    <aside className={`sidebar ${ui.mobileNavOpen ? 'sidebar-open' : ''}`}>
      ...
      <button type="button" className="workspace" onClick={() => ui.go('tasks')} title="回到任务中心">
