...
export function TaskTile({ task, onOpen }: { task: Task; onOpen: (code: string) => void }) {
  ...
  return (
    <button type="button" className="calm-task-card" onClick={() => onOpen(task.code)}>
      ...
      <h3>{task.title}</h3>
      <Tip label={taskPath(data, task).project}>
        <p className="task-belonging">{taskBelongingLabel(data, task)}</p>
      </Tip>
      <div className="calm-card-bottom">
        <Tip label={`负责人：${assignee}`}>
          <span>
            <Users size={14} />
            {assignee}
          </span>
        </Tip>
        <Tip label={`截止：${task.dueAt || '未设置'}`}>
          <span className={due.tone === 'red' ? 'due-overdue' : due.tone === 'amber' ? 'due-soon' : ''}>
            <Clock3 size={14} />
            {due.label}
          </span>
        </Tip>
      ...
      <div className="task-card-footer">
        ...
        <span className="task-card-counts">
          {records.length ? <Tip label={`${records.length} 条已发布迭代记录`}><span><GitBranch size={13} />{records.length}</span></Tip> : null}
          {task.githubLinks.length ? <Tip label={`${task.githubLinks.length} 个 GitHub 链接`}><span><Code2 size={13} />{task.githubLinks.length}</span></Tip> : null}
        </span>
      </div>
    </button>
  )
          ...
          {tasks.map(task => {
            ...
            return (
              <tr key={task.id}>
                ...
                <Tip label={path.module}><td>{taskBelongingLabel(data, task)}</td></Tip>
