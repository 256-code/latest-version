...
function AccountPanel() {
  ...
  return (
    <div className="popover-wrap">
      ...
      >
        <Tip label={`${user.name} · ${user.roleLabel}`}>
          <button
            type="button"
      ...
      <nav className="crumb" aria-label="面包屑导航">
        ...
        {showCatalogTrail && project ? (
          <>
            ...
            {module ? (
              <>
                <Tip label={project.name}>
                  <button type="button" onClick={() => ui.setCatalogLevel('project')}>{project.name}</button>
                </Tip>
                <ChevronRight size={14} aria-hidden="true" />
              </>
            ) : (
              <Tip label={project.name}>
                <strong aria-current="page">{project.name}</strong>
              </Tip>
            )}
            {feature && module ? (
              <>
                <Tip label={module.name}>
                  <button type="button" onClick={() => ui.setCatalogLevel('module')}>{module.name}</button>
                </Tip>
                <ChevronRight size={14} aria-hidden="true" />
